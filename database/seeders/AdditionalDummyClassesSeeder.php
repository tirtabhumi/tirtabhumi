...
class AdditionalDummyClassesSeeder extends Seeder
{
    public function run()
    {
        $user = User::first();
        ...
        foreach ($classTopics as $index => $topic) {
            // Create Training with timestamp to ensure unique slug
            $training = Training::create([
                'title' => $topic,
                'slug' => Str::slug($topic) . '-' . time() . '-' . $index,
        ...
        $this->command->info("\n✅ Successfully created 20 additional dummy classes for {$user->email}");
    }
